...
for {set i 1} {$i <= $nopulls} {incr i} {
    ...
    # Initialize angular restraints for suger (with respect to pulling path)
    set restrfile [subst $[subst restrFile$i]]
    set id [open $restrfile r]
    gets $id s1; gets $id s2
    close $id

    # Create namd2 groups
    lappend gs [addgroup $sel]
    lappend ts [addgroup $s1]; lappend ts [addgroup $s2]
    ...
    # Print headers
    print "HGP${i}1: \# TS   Theta"
    print "HGP${i}2: \# TS   R$i           R0$i          F$i"
    print "HGP${i}3: \# TS   Work   Nseg   Tseg   (for path $i)"
}
...
proc calcforces {} {
    global nopulls gs HGPk N0 step Nseg Tseg Maxsegs dR0s R0s outFreq works HGPkq ts
    ...
    set Fs {}
    for {set i 0; set i1 1; set t1 0; set t2 1} {$i < $nopulls} {incr i; incr i1; incr t1 2; incr t2 2} {
	...
	# angular constraint along z axis
	set p1     $coor([lindex $ts $t1])
	set p2     $coor([lindex $ts $t2])
	set r12    [vecsub $p1 $p2]
	set n12    [vecnorm $r12]
	set z      "0 0 1"
	set cost   [vecdot $n12 $z]
	set theta  [expr acos($cost)]
	set coef   [expr [veclength $r12] * sqrt(1 - $cost * $cost)]
	set f1     [vecscale [expr $HGPkq * $theta / $coef] [vecsub $z [vecscale $cost $n12]]]
	set f2     [vecscale -1 $f1]
	addforce   [lindex $ts $t1] $f1
	addforce   [lindex $ts $t2] $f2

	# output data 1
        if {$step % $outFreq == 0} {
	    print  "HGP${i1}1: $step [expr 180 * $theta / $pi]"
	    print  "HGP${i1}2: $step $R [lindex $R0s $i] $F"
	}
    }
	...
	if {$Nseg < [lindex $Maxsegs $i]} {
	    ...
	    # output data 2
	    if {$step % $outFreq == 0} {
		print "HGP${i1}3: $step $work $Nseg $Tseg"
	    }
	} else {
